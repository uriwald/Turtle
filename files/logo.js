...
// Additional work 2011 TurtleAcademy.com
//
...
//----------------------------------------------------------------------
{
    ...
    var regexIdentifier = /^(\.?[A-Za-zא-ת][A-Za-zא-ת0-9_.\?]*)(.*?)$/;
    ...
    //----------------------------------------------------------------------
    {
        ...
        throw gt.gettext("Don't know about variable") + " "+ name;
    };
        ...
        while (peek(list, ['=', '<', '>', '<=', '>=', '<>'])) {
            ...
            lhs = function(lhs) {
                ...
                switch (op) {
                    case "<":
                        return function() {
                            return (aexpr(lhs()) < aexpr(rhs())) ? 1 : 0;
                        };
                    case ">":
                        return function() {
                            return (aexpr(lhs()) > aexpr(rhs())) ? 1 : 0;
    ...
    //----------------------------------------------------------------------
    {
        ...
        while (peek(list, ['^'])) {
            ...
            lhs = function(lhs) {
                ...
                return function() {
                    return Math.pow(aexpr(lhs()), aexpr(rhs()));
                };
            } (lhs);
        }
    ...
    {
        if (!list.length) {
            throw gt.gettext("Unexpected end of instructions");
        }
    ...
    //----------------------------------------------------------------------
    {
        ...
        if (!procedure) {
            // TurtleAcademy
            // Checking if the command is in storage 
            if ( $.Storage.get(name) == undefined )   
            {
                throw gt.gettext("Don't know how to") + " "+ name;
            }
            else
            {
                var func = function() {
                ...
                var command = JSON.parse($.Storage.get(name));
                ...
                // Now we load it to the current procedure
                procedure = self.routines[name.toLowerCase()]; 
        ...
        if (procedure.noeval) {
            ...
            return function() {
                return procedure.apply(procedure, args.map(function(a) {
                    return a();
                }));
            };
        }
    ...
    function lexpr(atom) {
        ...
        if (Type(atom) === 'number') {
            return Array.prototype.map.call(atom.toString(), function(x) {
                return x;
            });
        }
        if (Type(atom) === 'word') {
            return Array.prototype.map.call(atom, function(x) {
                return x;
            });
        }
        if (Type(atom) === 'list') {
    ...
    function mapreduce(list, mapfunc, reducefunc, initial) {
        // NOTE: Uses Array.' : '', format to handle array-like types: arguments and strings
    ...
    self.routines["to"] = function(list) {
        ...
        if (!name.match(regexIdentifier)) {
            throw gt.gettext("Expected identifier");
        }
        ...
        while (list.length) {
            var atom = list.shift();
            if (Type(atom) === 'word' && (atom === 'end' || atom === gt.gettext('end'))) {
                break;
            }
        ...
        // TurtleAcademy
        // Save the learned command to localstorage
       
        var nameParams = JSON.stringify([inputs , block]);
        $.Storage.set( name , nameParams);
    //console.log("The method " + name + "was saved with the following input " + JSON.stringify(inputs) + " and the following block " + JSON.stringify(block) ) ;
    ...
    self.routines["def"] = function(list) {
        ...
        if (!proc.inputs) {
            throw "Can't show definition of intrinsic " + name;
        }

        var def = gt.gettext("to") + " " + name + " ";
        if (proc.inputs.length) {
            def += proc.inputs.map(function(a) {
                return ":" + a;
            }).join(" ");
            def += " ";
        }
    ...
    self.routines["word"] = function(word1, word2) {
        return arguments.length ? mapreduce(arguments, sexpr, function(a, b) {
            return a + " " + b;
        }) : "";
    };
    ...
    self.routines["reverse"] = function(list) {
        return lexpr(list).slice().reverse();
    };
    ...
    self.routines["firsts"] = function(list) {
        return lexpr(list).map(function(x) {
            return x[0];
        });
    };
    ...
    self.routines["emptyp"] = self.routines["empty?"] = function(thing) {
        return lexpr(thing).length === 0 ? 1 : 0;
    };
    ...
    function(thing, list) {
        return lexpr(list).some(function(x) {
            return self.equal(x, thing);
        }) ? 1 : 0;
    };
    ...
    self.routines["sum"] = function(a, b) {
        return mapreduce(arguments, aexpr, function(a, b) {
            return a + b;
        }, 0);
    };
    ...
    self.routines["power"] = function(a, b) {
        return Math.pow(aexpr(a), aexpr(b));
    };
    self.routines["sqrt"] = function(a) {
        return Math.sqrt(aexpr(a));
    };
    self.routines["exp"] = function(a) {
        return Math.exp(aexpr(a));
    };
    self.routines["log10"] = function(a) {
        return Math.log(aexpr(a)) / Math.LN10;
    };
    self.routines["ln"] = function(a) {
        return Math.log(aexpr(a));
    };
    ...
    self.routines["sin"] = function(a) {
        return Math.sin(deg2rad(aexpr(a)));
    };
    self.routines["cos"] = function(a) {
        return Math.cos(deg2rad(aexpr(a)));
    };
    self.routines["tan"] = function(a) {
        return Math.tan(deg2rad(aexpr(a)));
    };
    ...
    self.routines["radsin"] = function(a) {
        return Math.sin(aexpr(a));
    };
    self.routines["radcos"] = function(a) {
        return Math.cos(aexpr(a));
    };
    self.routines["radtan"] = function(a) {
        return Math.tan(aexpr(a));
    };

    self.routines["abs"] = function(a) {
        return Math.abs(aexpr(a));
    };
    ...
    self.routines["int"] = function(a) {
        return truncate(aexpr(a));
    };
    self.routines["round"] = function(a) {
        return Math.round(aexpr(a));
    };
    ...
    self.routines["true"] = function() {
        return 1;
    };
    self.routines["false"] = function() {
        return 0;
    };

    self.routines["and"] = function(a, b) {
        return Array.prototype.every.call(arguments, function(f) {
            return f();
        }) ? 1 : 0;
    };
    ...
    self.routines["or"] = function(a, b) {
        return Array.prototype.some.call(arguments, function(f) {
            return f();
        }) ? 1 : 0;
    };
    ...
    self.routines["setxy"] = function(x, y) {
        turtle.setposition(aexpr(x), aexpr(y));
    };
    self.routines["setx"] = function(x) {
        turtle.setposition(aexpr(x), undefined);
    }; // TODO: Replace with ...?
    self.routines["sety"] = function(y) {
        turtle.setposition(undefined, aexpr(y));
    };
    self.routines["setheading"] = self.routines["seth"] = function(a) {
        turtle.setheading(aexpr(a));
    };
    ...
    self.routines["pos"] = function() {
        var l = turtle.getxy();
        return [l[0], l[1]];
    };
    self.routines["xcor"] = function() {
        var l = turtle.getxy();
        return l[0];
    };
    self.routines["ycor"] = function() {
        var l = turtle.getxy();
        return l[1];
    };
    self.routines["heading"] = function() {
    ...
    self.routines["wrap"] = function() {
        turtle.setturtlemode('wrap');
    };
    self.routines["window"] = function() {
        turtle.setturtlemode('window');
    };
    self.routines["fence"] = function() {
        turtle.setturtlemode('fence');
    };
    ...
    self.routines["setlabelheight"] = function(a) {
        turtle.setfontsize(aexpr(a));
    };
    ...
    self.routines["local"] = function(varname) {
        ...
        Array.prototype.forEach.call(arguments, function(name) {
            localscope[sexpr(name).toLowerCase()] = undefined;
        });
    };
    ...
    self.routines["global"] = function(varname) {
        ...
        Array.prototype.forEach.call(arguments, function(name) {
            globalscope[sexpr(name).toLowerCase()] = undefined;
        });
    };
    ...
    self.routines["contents"] = function() {
        ...
        Object.keys(self.routines).filter(function(x) {
            return !self.routines[x].primitive;
        }),
        self.scopes.reduce(function(list, scope) {
            return list.concat(Object.keys(scope));
        }, [])
        ];
    };
    ...
    self.routines["names"] = function() {
        return [[], self.scopes.reduce(function(list, scope) {
            return list.concat(Object.keys(scope));
        }, [])];
    };
    ...
    self.routines["apply"] = function(procname, list) {
        ...
        if (!routine) {
            throw gt.gettext("Don't know how to") + " "+ procname;
        }
        if (routine.special) {
            throw "Can't apply over special " + procname;
        }
        if (routine.noeval) {
            throw "Can't apply over special " + procname;
        }
    ...
    self.routines["invoke"] = function(procname) {
        ...
        if (routine.special) {
            throw "Can't invoke over special " + procname;
        }
        if (routine.noeval) {
            throw "Can't invoke over special " + procname;
        }
    ...
    self.routines["foreach"] = function(procname, list) {
        ...
        if (routine.special) {
            throw "Can't foreach over special " + procname;
        }
        if (routine.noeval) {
            throw "Can't foreach over special " + procname;
        }
    ...
    self.routines["map"] = function(procname, list) {
        ...
        if (routine.special) {
            throw "Can't map over special " + procname;
        }
        if (routine.noeval) {
            throw "Can't map over special " + procname;
        }
    ...
    self.routines["filter"] = function(procname, list) {
        ...
        if (routine.special) {
            throw "Can't filter over special " + procname;
        }
        if (routine.noeval) {
            throw "Can't filter over special " + procname;
        }
    ...
    self.routines["reduce"] = function(procname, list) {
        ...
        if (procedure.special) {
            throw "Can't reduce over special " + procname;
        }
        if (procedure.noeval) {
            throw "Can't reduce over special " + procname;
        }
...
function __(str){
    var s;
    if(typeof(routines.translated) === 'object' && (s=routines.translated[str])) return s;
    return str;
}