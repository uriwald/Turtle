<?php

    echo _("forward"); echo _("fd"); echo _("Don't know how to"); 
    echo _("back"); echo _("to"); echo _("word"); echo _("list"); 
    echo _("sentence");  echo _("combine");  echo _("reverse"); 
    echo _("gensym"); echo _("first");   echo _("last"); 
    echo _("print");  echo _("sin"); echo _("cos");  
    echo _("tan"); 
    echo _("random");  echo _("true"); echo _("false");  echo _("and"); echo _("or");  echo _("xor"); 
    echo _("not");  echo _("bk");  echo _("lt");  echo _("left"); 
    echo _("rt");  echo _("right");  echo _("home");  echo _("st"); 
    
    
    
    echo _("showturtle");  echo _("ht");  echo _("hideturtle");  echo _("clean"); 
    echo _("cs");  echo _("clearscreen");  echo _("label");  echo _("fd"); 
    echo _("pendown");  echo _("pu");  echo _("penup");  echo _("penpaint"); 
    echo _("setcolor");  echo _("setpencolor");  echo _("setwidth");  echo _("setpensize"); 
    echo _("pencolor");  echo _("pensize");  echo _("repeat");  echo _("repcount"); 
    echo _("if");  echo _("ifelse");  echo _("iftrue");  echo _("iffalse"); 
    
    echo _("for");  echo _("end");  echo _("Unexpected end of instructions");  echo _("Expected identifier"); 
    echo _("Please don't try to override primitive functions");  echo _("Error");  echo _("hint");  echo _("sum"); 
        
    echo _("minus");  echo _("make"); echo _("pd"); echo _("stop");
    echo _("Hi") ; echo _("Welcome to the Turtle world"); 
    
    echo _("Please enter your username");echo _("Please enter your password");
    echo _("Your username must contain at least 4 characters");echo _("Your password must contain at least 5 characters");
    echo _("Add lesson step");echo _("Are you sure you want to delete the lesson");echo _("Your lessson has been deleted");
    echo _("successfully deleted");echo _("an error occured");echo _("Lesson title should contain at least 2 caracters");
    echo _("You must save the lesson before show lesson"); echo _("Please enter a valid email address"); 
    echo _("Having some problems to register");
    
    //New lesson
    echo _("setlabelheight");echo _("setheading"); echo _("seth");echo _("setxy");echo _("sety");echo _("setx");echo _("arc"); 
    echo _("pos"); echo _("xcor"); echo _("ycor"); echo _("heading"); echo _("towards");
    //Accessing list lesson commands
    echo _("item"); echo _("pick"); echo _("butfirst"); echo _("bf"); echo _("butlast"); echo _("bl"); 
    
    echo _("fill"); echo _("filled"); echo _("run"); echo _("lesson");echo _("difference");
    echo _("Command and number should be seperate by space e.g forward 50 and not forward50");
    
    echo _("black"); echo _("blue"); echo _("lime"); echo _("cyan");
    echo _("red"); echo _("magenta");echo _("yellow");echo _("white");echo _("brown");
    echo _("tan"); echo _("green");echo _("aquamarine");
    echo _("salmon");echo _("purple");echo _("orange");echo _("gray");
    
     
?>
  